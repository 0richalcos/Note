...
<img src="!assets/Linux/image-20250305005941399.png" alt="image-20250305005941399" style="zoom: 33%;" />
...
- `/srv/www/` 或 `/srv/http/`：存放网站文件。
- `/srv/ftp/`：存放 FTP 服务器的文件。
- `/srv/cvs/`：存放 CVS 版本控制仓库的数据。
...
- *[选项]*：
  ...
  - `-f <归档文件>`/`--file=<归档文件>`：使用档案名字，切记，这个参数是最后一个参数，后面只能接 *<归档文件>*。

- *<归档文件>*：要解压或者压缩的 `tar` 存档名称。
...
- *<文件系统类型>*：指定文件系统的类型，通常不必指定，因为 `mount` 会自动选择正确的类型。常用类型有：
  - `iso9660`：光盘或光盘镜像。
...
- *[选项]*：
  - `-u`：仅显示用户号（UID）。
  - `-g`：仅显示主组号（GID）。
  - `-G`：显示用户所属的所有用户组号（GID 列表）。
  - `-n`：以名称形式显示信息（需与 `-u`、`-g`、`-G` 配合使用）。
  - `-r`：显示真实用户 ID（real UID/GID），常用于 SUID 程序场景。
...
- `df`（英文全拼：disk full）：列出文件系统的整体磁盘使用量。
- `du`（英文全拼：disk used）：检查磁盘空间使用量。
- `fdisk`：用于磁盘分区。
...
- `-a` ：列出所有的文件与目录容量，因为默认仅统计目录底下的文件容量而已。
...
- 通配符 `*` 来代表每个目录。
- `2>/dev/null` 是为了屏蔽权限拒绝的报错。
- `sort -hr` 是按人类可读的大小降序排列。
...
- Debian 系：软件包后缀为 `.deb`，底层管理工具是 dpkg (Debian Package)。
- RedHat 系：软件包后缀为 `.rpm`，底层管理工具是 rpm (Red Hat Package Manager)。
...
- 将该软件安装在 `/opt/sudytech/apache2` 下面。
- 执行文件安装在 `/opt/sudytech/apache2/bin` （而不是默认的 `/usr/local/bin`）。
- 资源文件安装在 `/opt/sudytech/apache2/share`（而不是默认的 `/usr/local/share`）。
...
- `-i`（英文全拼 install）：安装一个本地的 `.deb` 文件。
- `-r`（英文全拼 remove）：卸载软件，但保留配置文件。
- `-P`（英文全拼 Purge）：卸载软件，同时删除所有配置文件。
- `-l`（英文全拼 list）：列出当前系统已安装的软件列表。
...
- `-ivh`（英文全拼 install verbose hash）：安装一个本地的 `.rpm` 文件。
- `-e`（英文全拼 erase）：卸载软件（删除软件本身，但可能保留配置文件）。
- `-q`（英文全拼 query）：查询已安装软件的信息（版本、依赖等）。
- `-qa`（英文全拼 query all）：列出系统中所有已安装的软件包。
...
- 查询相关：
  ...
  - 查找软件包命令： `sudo yum search <keyword>`。
  - 显示软件包具体信息，例如：版本号，安装大小，依赖关系等等：`sudo yum info <package_name>`。

- 安装相关：

  - 列出所有已安装的包：`yum list installed`。
  - 安装指定的软件命令：`sudo yum install <package_name>`。
  - 安装多个软件包：`sudo yum install <package_1> <package_2> <package_3>`。
  - 重新安装软件包：`sudo yum reinstall <package_name>`。

- 更新相关：

  - 列出所有可更新的软件清单命令：`yum check-update`。

  - 更新指定的软件命令：`sudo yum update <package_name>`。

  - 升级所有软件包：`sudo yum update`。

    > [!IMPORTANT]
    >
    > `yum update` 会同时更新软件和系统内核。若只想更新软件不更新内核，需谨慎操作

- 删除相关：

  - 删除软件包命令：`sudo yum remove <package_name>`。
...
- *<信息编号>*：若不加 `<信息编号>` 选项，则 `-l` 参数会列出全部的信息名称。
- *<信息名称或编号>*：指定要送出的信息。
...
- `-H`/ `--header <header>`：设置请求头。可以多次使用该参数添加多个头。
- `-k`/`--insecure`：忽略 SSL 证书验证。允许连接到 SSL 证书无效（如自签名）的 HTTPS 网站。
- `-L`/`--location`：跟随重定向。如果服务器返回 3xx 重定向，`curl` 会自动请求新的地址。
- `-o`/ `--output`：把输出写到该文件中
