...
<img src="!assets/Mybatis/20200605012013.png" alt="20200605012013" style="" />
...
<img src="!assets/Mybatis/image-20200614172418322.png" alt="image-20200614172418322" style="" />
...
<img src="!assets/Mybatis/image-20200614172730202.png" alt="image-20200614172730202" style="" />
...
构造函数方式注入允许你在初始化时为类设置属性的值，而不用暴露出公有方法。MyBatis 也支持私有属性和私有 JavaBean 属性来完成注入，但有一些人更青睐于通过构造函数进行注入。 `constructor` 元素就是为此而生的。
...
关联（association）元素处理 “有一个” 类型的关系。 比如在示例中，一个博客有一个用户。关联结果映射和其它类型的映射工作方式差不多。 你需要指定目标属性名以及属性的`javaType`（很多时候 MyBatis 可以自己推断出来），在必要的情况下你还可以设置 JDBC 类型，如果你想覆盖获取结果值的过程，还可以设置类型处理器。
...
`association` 元素的基本属性和普通的结果映射相比，它只在 `select` 和 `resultMap` 属性上有所不同：
...
这个问题会导致成百上千的 SQL 语句被执行。好消息是，MyBatis 能够对这样的查询进行延迟加载，因此可以将大量语句同时运行的开销分散开来。 然而，如果你加载记录列表之后立刻就遍历列表以获取嵌套的数据，就会触发所有的延迟加载查询，性能可能会变得很糟糕。
...
现在将博客表和作者表连接在一起，而不是执行一个独立的查询语句，就像这样：
...
现在，上面的示例使用了外部的结果映射元素来映射关联。这使得 Author 的结果映射可以被重用。 如果不打算重用它，或者你更喜欢将你所有的结果映射放在一个具有描述性的结果映射元素中，可以直接将结果映射作为子元素嵌套在内。这里给出使用这种方式的等效例子：
...
现在可以指定使用 “authors” 结果集的数据来填充 “author” 关联：
...
要像上面这样，映射嵌套结果集合到一个 List 中，可以使用集合元素。 和关联元素一样，可以使用嵌套 Select 查询，或基于连接的嵌套结果映射集合。
...
大部分都和上面的关联元素非常相似，集合有一个新的 `ofType` 属性，这个属性非常重要，它用来将 JavaBean（或字段）属性的类型和集合存储的类型区分开来。 所以可以按照下面这样来阅读映射： “posts 是一个存储 Post 的 ArrayList 集合”。

在一般情况下，MyBatis 可以推断 `javaType` 属性，因此并不需要填写。所以很多时候可以简略成：
...
如果你喜欢更详略的、可重用的结果映射，可以使用下面的等价形式：
...
<img src="!assets/Mybatis/image-20200616093839656.png" alt="image-20200616093839656" style="" />
...
<img src="!assets/Mybatis/QQ截图20200616101536.png" alt="" style="" />
